# General Stream Basic Daemon

A live-mode blueprint that continuously sends telemetry chunks between agents using MirrorNeuron stream messages until the job is manually cancelled.
...
The `telemetry_source` emits gzipped NDJSON chunks with `class: stream`, `content_type: application/x-ndjson`, and stream metadata on every message. It never sends `close` or `eof` stream markers. The `peak_detector` is a stream consumer that keeps rolling state and emits `stream_peak_detected` events when generated throughput spikes cross the threshold.
