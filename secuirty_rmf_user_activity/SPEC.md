# Security RMF User Activity Response Worker SPEC

## What We Want To Achieve

Build a reviewable security and governance workflow that helps Security-conscious SaaS, GovTech, defense-tech, compliance, and platform teams move from raw signals to an explainable recommendation. Detects suspicious user activity, recommends safe step-up authentication, and generates RMF/ATO/cATO-ready evidence artifacts. The target customer should understand what changed, why the system recommended an action, and what evidence a human should review before acting.

## Customer Problem

User activity streams need stateful triage, safe response instructions, and audit-ready evidence tied to source events. In a real customer environment, the pain is not only producing an answer; it is preserving context across changing inputs, exposing tradeoffs, and creating an audit trail that business, technical, or governance stakeholders can trust.

## Design Details

The blueprint is organized as a MirrorNeuron workflow with stable identity, configurable inputs, structured events, and a final artifact. The main agent role is security activity triage and evidence agent. The workflow uses a configurable MirrorNeuron workflow and demonstrates event stream normalization, state machine, deterministic risk scoring, dry-run response policy, and candidate compliance evidence artifacts.
...
The expected customer outcome is structured security response evidence, including a security decision, evidence report, and incident summary. A useful run should show the starting context, the observations made during the workflow, the action or recommendation rationale, and the final artifact that a domain owner can review.
...
When using the local run store, inspect `outputs/security_decision.json`, `outputs/evidence_report.json`, and `outputs/incident_summary.md`. These artifacts are the review surface for debugging the workflow, comparing scenarios, and deciding whether the blueprint is ready for a real adapter.
