# Climate Resilience Planning Assistant

`Blueprint ID:` `climate_resilience_planning_assistant`
`Category:` `Science`
...
```bash
mn run climate_resilience_planning_assistant
```
...
Run from the catalog with `mn run climate_resilience_planning_assistant` or from this folder with `mn run --folder .`.
