# General Stream Live Backpressure Daemon

This blueprint demonstrates MirrorNeuron as a never-ending live event runtime with bounded stream pressure instead of an unlimited task queue. It keeps generating stream data until you cancel the job manually.

The graph is intentionally small:

- `ingress`: accepts the initial stream start signal.
- `live_source`: emits generated telemetry events forever in stream mode.
...
```bash
cd MirrorNeuron
MIRROR_NEURON_GRPC_PORT=55251 mix run ../mn-blueprints/general_stream_live_backpressure_deamon/demo_backpressure.exs
```
...
- `live_stream_event_generated`: the live source generated another stream event.
