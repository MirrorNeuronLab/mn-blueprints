# Supply Chain Resilience Assistant

`Blueprint ID:` `supply_chain_resilience_assistant`
`Category:` `Business`

Use this blueprint to rehearse supplier disruption responses and choose actions that protect inventory, fulfillment, and customer service levels.
...
```bash
mn run supply_chain_resilience_assistant
```
...
Run from the catalog with `mn run supply_chain_resilience_assistant` or from this folder with `mn run --folder .`.
