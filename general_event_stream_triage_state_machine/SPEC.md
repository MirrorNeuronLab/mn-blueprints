# Event Stream Triage State Machine SPEC

## What We Want To Achieve

Build a reviewable runtime evaluation workflow that helps Developers building alerting, stream processing, and incident triage agents move from raw signals to an explainable recommendation. Triage event streams as anomaly pressure, queue depth, and false positives change over time. The target customer should understand what changed, why the system recommended an action, and what evidence a human should review before acting.

## Customer Problem

Event streams are noisy and bursty; teams need agents that maintain state and route actions as conditions change. In a real customer environment, the pain is not only producing an answer; it is preserving context across changing inputs, exposing tradeoffs, and creating an audit trail that business, technical, or governance stakeholders can trust.

## Design Details

The blueprint is organized as a MirrorNeuron workflow with stable identity, configurable inputs, structured events, and a final artifact. The main agent role is Event stream triage agent. The workflow uses stateful event stream simulation and demonstrates event stream processing, state machine, LLM triage, and feedback loop.
...
A representative scenario is: An event stream spikes, anomaly scores rise, and the triage agent tightens filters or escalates high-signal events.
...
Important state inputs include `event_rate`, `anomaly_score`, `queue_depth`, and `false_positive_rate`. Where the blueprint uses an action loop, the current action space includes `escalate_anomalies`, `tighten_filter`, and `hold_policy`. For production use, the same contract should be fed by customer system-of-record data, business rules, approval policies, thresholds, and any regulated or safety-critical constraints needed for the operating environment.

## Output: Expected Customer Outcome

The expected customer outcome is event stream report with queue state, triage actions, and ranked partitions. A useful run should show the starting context, the observations made during the workflow, the action or recommendation rationale, and the final artifact that a domain owner can review.
...
- State trajectory: inspect whether `event_rate`, `anomaly_score`, `queue_depth`, and `false_positive_rate` move coherently across the workflow rather than appearing as disconnected summaries.
