defmodule MirrorNeuron.Skills.SlackCommunicateSkill do
  ...
  def send_message(text, opts) when is_binary(text) do
    ...
    token = option_or_env(opts, :token, ["MN_SLACK_BOT_TOKEN", "SLACK_BOT_TOKEN"])
    channel = option_or_env(opts, :channel, ["MN_SLACK_DEFAULT_CHANNEL", "SLACK_DEFAULT_CHANNEL"])
    api_url = option_or_env(opts, :api_url, ["MN_SLACK_API_BASE_URL", "SLACK_API_BASE_URL"], @default_api_url)
