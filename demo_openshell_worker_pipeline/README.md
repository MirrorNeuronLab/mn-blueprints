# Demo: OpenShell Worker Pipeline

`Blueprint ID:` `demo_openshell_worker_pipeline`
...
```bash
mn run demo_openshell_worker_pipeline
```
...
Run from the catalog with `mn run demo_openshell_worker_pipeline` or from this folder with `mn run --folder .`.
