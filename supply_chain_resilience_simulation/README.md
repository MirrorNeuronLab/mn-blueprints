...
`Blueprint ID:` `supply_chain_resilience_simulation`
`Category:` `Business`
...
```bash
mn run supply_chain_resilience_simulation
```
...
Run from the catalog with `mn run supply_chain_resilience_simulation` or from this folder with `mn run --folder .`.
