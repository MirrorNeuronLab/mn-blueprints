# AI Strategy Advisory Assistant

`Blueprint ID:` `ai_strategy_advisory_assistant`
`Category:` `Business`
...
```bash
mn run ai_strategy_advisory_assistant
```
...
Run from the catalog with `mn run ai_strategy_advisory_assistant` or from this folder with `mn run --folder .`.
