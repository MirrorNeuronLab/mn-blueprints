# Demo: Context Memory Compression

`Blueprint ID:` `demo_context_memory_compression`
`Category:` `Engineering`

Use this blueprint to test whether compressed working memory keeps a growing LLM workflow useful while reducing context load and cost.
...
```bash
mn run demo_context_memory_compression
```
...
Run from the catalog with `mn run demo_context_memory_compression` or from this folder with `mn run --folder .`.
