# Facility Safety Video Monitor

`Blueprint ID:` `facility_safety_video_monitor`
`Category:` `Security`
...
```bash
mn run facility_safety_video_monitor
```
...
Run from the catalog with `mn run facility_safety_video_monitor` or from this folder with `mn run --folder .`.
