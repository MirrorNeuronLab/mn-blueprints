# Pricing Profit Optimization Assistant

`Blueprint ID:` `pricing_profit_optimization_assistant`
`Category:` `Business`

Use this blueprint to compare pricing moves against demand, inventory, margin, and competitors before you change prices in the real world.
...
```bash
mn run pricing_profit_optimization_assistant
```
...
Run from the catalog with `mn run pricing_profit_optimization_assistant` or from this folder with `mn run --folder .`.
