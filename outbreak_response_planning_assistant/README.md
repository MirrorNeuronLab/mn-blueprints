# Outbreak Response Planning Assistant

`Blueprint ID:` `outbreak_response_planning_assistant`
`Category:` `Science`

Use this blueprint to compare outbreak response policies as infections, mobility, vaccination, and hospital load evolve.
...
```bash
mn run outbreak_response_planning_assistant
```
...
Run from the catalog with `mn run outbreak_response_planning_assistant` or from this folder with `mn run --folder .`.
