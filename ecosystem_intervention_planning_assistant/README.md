# Ecosystem Intervention Planning Assistant

`Blueprint ID:` `ecosystem_intervention_planning_assistant`
`Category:` `Science`
...
```bash
mn run ecosystem_intervention_planning_assistant
```
...
Run from the catalog with `mn run ecosystem_intervention_planning_assistant` or from this folder with `mn run --folder .`.
