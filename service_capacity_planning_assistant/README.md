# Service Capacity Planning Assistant

`Blueprint ID:` `service_capacity_planning_assistant`
`Category:` `Business`

Use this blueprint to test staffing, deflection, and escalation decisions before service queues miss SLA targets.
...
```bash
mn run service_capacity_planning_assistant
```
...
Run from the catalog with `mn run service_capacity_planning_assistant` or from this folder with `mn run --folder .`.
