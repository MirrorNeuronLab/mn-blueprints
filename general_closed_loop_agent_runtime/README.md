...
`Blueprint ID:` `general_closed_loop_agent_runtime`  
...
Show the core runtime pattern: an LLM agent observes a changing queue and acts over repeated ticks.
...
Developers, evaluators, and technical buyers learning the closed-loop agent model.

## Why it matters

Most LLM demos stop after one answer; real operations require repeated observations, decisions, state updates, and traceable outcomes. A static dashboard can show the current state, and a one-shot LLM prompt can summarize it, but neither tests what happens after a decision is applied. This blueprint makes the feedback loop visible: state changes, the agent observes it, the agent chooses an action, and the system evolves again.
...
1. Load mock or adapter-provided inputs for time-stepped operations queue simulation.
2. Initialize time-varying state with metrics such as `backlog`, `capacity`, `quality_score`, `operating_cost`.
3. Advance the environment one step with deterministic seeded drift and volatility.
4. Ask the `Operations loop controller` to observe the current state, compare valid actions, and choose among `increase_capacity`, `focus_quality`, `hold_policy`.
5. Apply the selected action back into the simulated system so the next observation changes.
6. Write a structured final artifact for `simulation summary` with action history, state deltas, ranked options, and next steps.

## Example scenario

A work queue grows, capacity and quality drift, and an operations agent chooses whether to add capacity, protect quality, or hold policy.
...
| `initial_backlog` | Scenario control, seed, or domain input. | 18 | Yes |
| `initial_backlog` | Override the starting value for open work backlog. | 18 | Yes |
| `initial_capacity` | Override the starting value for available service capacity. | 14 | Yes |
| `initial_quality_score` | Override the starting value for output quality score. | 84 | Yes |
| `initial_operating_cost` | Override the starting value for operating cost index. | 22 | Yes |
...
| `final_artifact` | The user-facing simulation summary with action history, state deltas, and next steps. | `recommended_action`, `ranked_options`, `next_steps` |
...
```bash
cd general_closed_loop_agent_runtime
...
Replace queue metrics with your workflow KPIs, add actions, change the scoring heuristic, and connect the final artifact to a dashboard.
...
Inspect `timeline` to see each observation and decision, `state_changes` to verify the simulated `backlog` and `quality_score` moved over time, and `final_artifact` to review the recommended action and ranked work stream options.
...
This is the smallest proof of the platform thesis: agents reason inside systems, not outside them. The product lesson is that this is not just a chatbot around data. It is a repeatable pattern for vertical workflows where simulation, state, and agent decisions create a more defensible user experience than static analytics alone.
