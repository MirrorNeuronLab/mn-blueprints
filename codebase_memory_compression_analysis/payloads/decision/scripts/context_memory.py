...
def compile_context(
    ...
    actual_use_model_compression = (
        use_model_compression
        if use_model_compression is not None
        else env_bool("MN_CONTEXT_USE_MODEL_COMPRESSION", default=False)
    )
    ...
    response = stub.CompileContext(
        context_pb2.CompileContextRequest(
            ...
            use_model_compression=actual_use_model_compression,
            objective=objective,
...
def compile_context_state(compiled):
    trace = compiled.get("trace", {})
    input_tokens = compiled.get("estimated_input_tokens", 0) or 0
    output_tokens = compiled.get("estimated_output_tokens", 0) or 0
    target_tokens = compiled.get("target_tokens", 0) or 0
    token_budget = compiled.get("token_budget", 0) or 0
    compression_ratio = output_tokens / input_tokens if input_tokens else 1.0
    budget_limit = target_tokens or token_budget
    within_budget = output_tokens <= budget_limit if budget_limit else True
    latency_seconds = compiled.get("latency_seconds", 0.0) or 0.0
    return {
        ...
        "token_reduction": 1.0 - compression_ratio,
        "target_tokens": target_tokens,
        "token_budget": token_budget,
        "within_budget": within_budget,
        "budget_score": 1.0 if within_budget or compiled.get("warnings") else 0.0,
        "latency_seconds": latency_seconds,
        "input_tokens_per_second": input_tokens / latency_seconds if latency_seconds > 0 else None,
        "output_tokens_per_second": output_tokens / latency_seconds if latency_seconds > 0 else None,
        "max_items": compiled.get("max_items", 0),
        "use_model_compression": compiled.get("use_model_compression", False),
...
def _contains(text, needle):
    return str(needle or "") in text if needle else True
...
def benchmark_event(
    ...
    items = items or []
    output_text = json.dumps(output_payload or {}, sort_keys=True, default=str)
    terms = [term for term in (expected_terms or []) if term]
    refs = [ref for ref in (expected_source_refs or []) if ref]
    found_terms = [term for term in terms if _contains(output_text, term)]
    found_refs = [ref for ref in refs if _contains(output_text, ref)]
    compile_state = compile_context_state(compiled) if compiled else {}
    event = {
        "role": role,
        "stage": stage,
        "retrieved_item_count": len(items),
        "retrieved_artifact_types": [
            item.get("artifact_type") or item.get("type") for item in items
        ],
        "compile": compile_state,
        "output_tokens_estimate": estimate_tokens(output_payload or {}),
        "expected_terms_found": len(found_terms),
        "expected_terms_total": len(terms),
        "pinned_term_recall": _ratio(len(found_terms), len(terms)),
        "source_refs_found": len(found_refs),
        "source_refs_total": len(refs),
        "source_ref_recall": _ratio(len(found_refs), len(refs)),
    }
    if started_at is not None:
        event["agent_wall_seconds"] = time.perf_counter() - started_at
    if extra:
        event.update(extra)
    return event


def append_benchmark_event(source, event):
    events = list(source.get("benchmark_events", []))
    events.append(event)
    return events
...
def aggregate_benchmark_events(events):
    compile_states = [event.get("compile", {}) for event in events]
    input_tokens = sum(int(state.get("estimated_input_tokens") or 0) for state in compile_states)
    output_tokens = sum(int(state.get("estimated_output_tokens") or 0) for state in compile_states)
    output_artifact_tokens = sum(int(event.get("output_tokens_estimate") or 0) for event in events)
    latencies = [state.get("latency_seconds") for state in compile_states if state.get("latency_seconds") is not None]
    wall_times = [event.get("agent_wall_seconds") for event in events if event.get("agent_wall_seconds") is not None]
    return {
        "agent_count": len(events),
        "estimated_compile_input_tokens": input_tokens,
        "estimated_compile_output_tokens": output_tokens,
        "estimated_agent_output_tokens": output_artifact_tokens,
        "estimated_total_tokens_processed": input_tokens + output_tokens + output_artifact_tokens,
        "mean_compression_ratio": output_tokens / input_tokens if input_tokens else 1.0,
        "mean_token_reduction": 1.0 - (output_tokens / input_tokens) if input_tokens else 0.0,
        "budget_score": _ratio(
            sum(1 for state in compile_states if state.get("budget_score", 1.0) >= 1.0),
            len(compile_states),
        ),
        "compile_latency_seconds_total": sum(float(value or 0.0) for value in latencies),
        "compile_latency_seconds_mean": sum(float(value or 0.0) for value in latencies) / len(latencies) if latencies else None,
        "compile_latency_seconds_p50": percentile(latencies, 50),
        "compile_latency_seconds_p95": percentile(latencies, 95),
        "agent_wall_seconds_total": sum(float(value or 0.0) for value in wall_times),
        "agent_wall_seconds_mean": sum(float(value or 0.0) for value in wall_times) / len(wall_times) if wall_times else None,
        "tokens_per_second_compile_input": input_tokens / sum(float(value or 0.0) for value in latencies)
        if latencies and sum(float(value or 0.0) for value in latencies) > 0
        else None,
    }
...
def call_llm(system_prompt, user_prompt, mock_response):
    quick_mode = os.environ.get("MN_BLUEPRINT_QUICK_TEST", "1").strip().lower() in {"1", "true", "yes", "on"}
    if quick_mode:
