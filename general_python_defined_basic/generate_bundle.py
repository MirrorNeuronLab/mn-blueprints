...
from workflow import GeneralPythonDefinedBasic
from mn_sdk import workflow


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate a pure-Python SDK-defined bundle.")
    ...
    log_status(
        "general_python_defined_basic",
        "generating pure Python workflow bundle",
        phase="generate",
        details={"quick_test": quick_test},
    )

    bundle_dir = workflow.to_bundle(
        GeneralPythonDefinedBasic,
        args.output_dir,
        blueprint_id="general_python_defined_basic",
        includes=["existing_research.py"],
