# Robot Motion Planning Assistant

`Blueprint ID:` `robot_motion_planning_assistant`
`Category:` `Science`

Use this blueprint to visualize shared-world motion planning so you can inspect coordination, conflicts, and route choices between agents.
...
```bash
mn run robot_motion_planning_assistant
```
...
Run from the catalog with `mn run robot_motion_planning_assistant` or from this folder with `mn run --folder .`.
