...
SERVER_LOG="${SERVER_LOG:-/tmp/facility_safety_monitor_assistant_mediamtx.log}"
...
OPENSHELL_FRAME_BRIDGE_LOG="${OPENSHELL_FRAME_BRIDGE_LOG:-/tmp/facility_safety_monitor_assistant_openshell_frame_bridge.log}"
...
OPENSHELL_TUNNEL_LOG="${OPENSHELL_TUNNEL_LOG:-/tmp/facility_safety_monitor_assistant_openshell_rtsp_tunnel.log}"

usage() {
  cat <<EOF
Start a local RTSP stream from this Mac's webcam for Facility Safety Monitor Assistant.
...
start_rtsp_server() {
  ...
  config_dir="$(mktemp -d /tmp/facility_safety_monitor_assistant_mediamtx.XXXXXX)"
...
start_openshell_rtsp_tunnel() {
  ...
  tunnel_config_dir="$(mktemp -d /tmp/facility_safety_monitor_assistant_openshell_tunnel.XXXXXX)"
...
start_openshell_frame_bridge() {
  ...
  frame_bridge_dir="$(mktemp -d /tmp/facility_safety_monitor_assistant_frames.XXXXXX)"
