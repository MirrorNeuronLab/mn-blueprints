# Demo: Parallel Worker Benchmark

`Blueprint ID:` `demo_parallel_worker_benchmark`
`Category:` `Engineering`

Use this blueprint to stress-test broad parallel fan-out and measure how deterministic workers behave as runtime scale increases.
...
```bash
mn run demo_parallel_worker_benchmark
```
...
Run from the catalog with `mn run demo_parallel_worker_benchmark` or from this folder with `mn run --folder .`.
