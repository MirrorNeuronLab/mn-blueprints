# Vendor Selection Advisor Assistant

`Blueprint ID:` `vendor_selection_advisor_assistant`
`Category:` `Business`
...
```bash
mn run vendor_selection_advisor_assistant
```
...
Run from the catalog with `mn run vendor_selection_advisor_assistant` or from this folder with `mn run --folder .`.
