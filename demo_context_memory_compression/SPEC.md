# Demo: Context Memory Compression SPEC

## What We Want To Achieve

Build a reviewable runtime evaluation workflow that helps Developers building long-context agent systems with bounded prompt budgets move from raw signals to an explainable recommendation. Stress-test compressed working memory across a growing multi-agent LLM workflow. The target customer should understand what changed, why the system recommended an action, and what evidence a human should review before acting.

## Customer Problem

Long-running agents accumulate more context than a model can safely consume; compression must preserve pinned facts, constraints, and decision traces. In a real customer environment, the pain is not only producing an answer; it is preserving context across changing inputs, exposing tradeoffs, and creating an audit trail that business, technical, or governance stakeholders can trust.

## Design Details

The blueprint is organized as a MirrorNeuron workflow with stable identity, configurable inputs, structured events, and a final artifact. The main agent role is Context researcher, expander, challenger, compressor, briefing author. The workflow uses growing memory graph with compression pressure and demonstrates CompileContext, context compression, typed memory, and multi-agent LLM relay.
...
A representative scenario is: Agents create larger artifacts over several turns while CompileContext keeps each packet bounded and traceable.
...
The expected customer outcome is compressed context handoff and final briefing. A useful run should show the starting context, the observations made during the workflow, the action or recommendation rationale, and the final artifact that a domain owner can review.
...
Change token budgets, pinned terms, memory item types, compression policy, and model-compression settings. Add customer-specific policies, review gates, exception handling, retention rules, and monitoring dashboards. Calibrate the workflow against historical data and expert judgment, then track acceptance rate, correction rate, latency, incident reduction, cost impact, and other outcome metrics that prove whether the workflow is helping.

## Product Narrative

State management and compression are differentiators for agent platforms moving beyond one-shot prompts.
