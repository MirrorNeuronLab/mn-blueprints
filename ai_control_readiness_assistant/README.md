# AI Control Readiness Assistant

`Blueprint ID:` `ai_control_readiness_assistant`
`Category:` `Security`
...
```bash
mn run ai_control_readiness_assistant
```
...
Run from the catalog with `mn run ai_control_readiness_assistant` or from this folder with `mn run --folder .`.
