# Credit Default Early Warning System SPEC

## What We Want To Achieve

Build a reviewable financial decision-support workflow that helps Credit-risk teams, lenders, fintech underwriters, and portfolio analysts move from raw signals to an explainable recommendation. Detect rising borrower default pressure and compare credit policy responses over time. The target customer should understand what changed, why the system recommended an action, and what evidence a human should review before acting.

## Customer Problem

Borrower behavior, rates, approvals, and yield interact over time; static scorecards miss policy tradeoffs under changing conditions. In a real customer environment, the pain is not only producing an answer; it is preserving context across changing inputs, exposing tradeoffs, and creating an audit trail that business, technical, or governance stakeholders can trust.

## Design Details

The blueprint is organized as a MirrorNeuron workflow with stable identity, configurable inputs, structured events, and a final artifact. The main agent role is Credit risk decision agent. The workflow uses borrower default and credit policy simulation and demonstrates credit risk simulation, borrower behavior, policy decisions, and structured report.
...
A representative scenario is: Default rates rise in a consumer installment portfolio, and the agent weighs tightening credit against risk-based pricing.
...
Important state inputs include `default_rate_pct`, `interest_rate_pct`, `approval_rate_pct`, and `portfolio_yield_pct`. Where the blueprint uses an action loop, the current action space includes `tighten_credit`, `risk_based_pricing`, and `request_more_docs`. For production use, the same contract should be fed by customer system-of-record data, business rules, approval policies, thresholds, and any regulated or safety-critical constraints needed for the operating environment.

## Output: Expected Customer Outcome

The expected customer outcome is credit risk decision report with policy action history. A useful run should show the starting context, the observations made during the workflow, the action or recommendation rationale, and the final artifact that a domain owner can review.
...
- State trajectory: inspect whether `default_rate_pct`, `interest_rate_pct`, `approval_rate_pct`, and `portfolio_yield_pct` move coherently across the workflow rather than appearing as disconnected summaries.
...
Replace synthetic metrics with bureau attributes, payment history, application data, macro variables, and institution-specific approval policies. Add customer-specific policies, review gates, exception handling, retention rules, and monitoring dashboards. Calibrate the workflow against historical data and expert judgment, then track acceptance rate, correction rate, latency, incident reduction, cost impact, and other outcome metrics that prove whether the workflow is helping.

## Product Narrative

Credit risk is a regulated, high-ROI vertical where stateful AI can support explainable early-warning workflows.
