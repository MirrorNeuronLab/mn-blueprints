...
`Blueprint ID:` `native_live_monitor_service`
...
```bash
mn run native_live_monitor_service
```
...
Run from the catalog with `mn run native_live_monitor_service` or from this folder with `mn run --folder .`.
