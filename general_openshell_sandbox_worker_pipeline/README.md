# Openshell Sandbox Worker Pipeline

`Blueprint ID:` `general_openshell_sandbox_worker_pipeline`  
...
Run isolated shell and Python workers as a traceable runtime pipeline.
...
1. Load the graph in `manifest.json` and start `shell_worker` with bundled mock inputs.
2. Route work through the agents described by the manifest, using static worker pipeline with deterministic generated input as the evolving system.
3. Let the `Sandboxed worker executor` observe intermediate state, produce decisions or artifacts, and emit typed messages.
4. Preserve execution metadata, logs, and generated artifacts so users can audit what happened.
5. Return worker metrics and a python-generated report for review, customization, or downstream workflow integration.

## Example scenario

A developer sends a text payload to a shell metrics worker, routes the result into a Python report worker, and receives a structured completion event.

## Inputs

| Input | What it controls | Example | Can customize? |
|---|---|---|---|
| `manifest.json` initial inputs | Sample payloads routed into shell_worker. | `initial_inputs` | Yes |
...
| Final artifact | The user-facing worker metrics and a python-generated report. | `result.json`, report, alert, or generated artifact |
...
Replace the word-count scripts with domain workers, add policies for allowed egress, tune pool slots, and extend the output schema for your artifacts.
...
This shows that the runtime can orchestrate ordinary enterprise tools safely, which is essential for deploying agent workflows inside controlled environments. The product lesson is that this is not just a chatbot around data. It is a repeatable pattern for vertical workflows where simulation, state, and agent decisions create a more defensible user experience than static analytics alone.
