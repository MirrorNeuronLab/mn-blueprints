# Demo: Human Approval Gate

`Blueprint ID:` `demo_human_approval_gate`
`Category:` `Security`

Use this blueprint to add a human approval checkpoint before an agent applies high-impact actions, with clear review and audit records.
...
```bash
mn run demo_human_approval_gate
```
...
Run from the catalog with `mn run demo_human_approval_gate` or from this folder with `mn run --folder .`.
