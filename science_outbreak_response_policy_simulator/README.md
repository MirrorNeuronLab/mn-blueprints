# Outbreak Response Policy Simulator

`Blueprint ID:` `science_outbreak_response_policy_simulator`  
...
Compare outbreak interventions as infections, mobility, vaccination, and hospital load change.
...
1. Load mock or adapter-provided inputs for disease spread and intervention simulation.
2. Initialize time-varying state with metrics such as `infected_people`, `vaccination_pct`, `mobility_index`, `hospital_load_pct`.
3. Advance the environment one step with deterministic seeded drift and volatility.
4. Ask the `Public-health intervention advisor` to observe the current state, compare valid actions, and choose among `vaccination_push`, `reduce_mobility`, `increase_testing`.
5. Apply the selected action back into the simulated system so the next observation changes.
6. Write a structured final artifact for `public health intervention plan` with action history, state deltas, ranked options, and next steps.

## Example scenario

Infections rise in a 50,000-person population, and the advisor chooses vaccination, mobility reduction, or testing interventions.
...
| `population` | Scenario control, seed, or domain input. | 50000 | Yes |
| `initial_infected_people` | Override the starting value for currently infected people. | 180 | Yes |
| `initial_vaccination_pct` | Override the starting value for vaccination percent. | 42 | Yes |
| `initial_mobility_index` | Override the starting value for mobility index. | 78 | Yes |
| `initial_hospital_load_pct` | Override the starting value for hospital load percent. | 61 | Yes |
...
| `final_artifact` | The user-facing public-health intervention plan with state trajectory. | `recommended_action`, `ranked_options`, `next_steps` |
...
```bash
cd science_outbreak_response_policy_simulator
...
Replace synthetic disease state with surveillance, mobility, vaccination, hospital, or wastewater data; tune transmission and intervention assumptions.
...
Inspect `timeline` to see each observation and decision, `state_changes` to verify the simulated `infected_people` and `hospital_load_pct` moved over time, and `final_artifact` to review the recommended action and ranked district options.
...
Public-sector and healthcare planning need explainable scenario tools; this shows an AI advisor inside a dynamic disease model. The product lesson is that this is not just a chatbot around data. It is a repeatable pattern for vertical workflows where simulation, state, and agent decisions create a more defensible user experience than static analytics alone.
