# Revenue Retention Simulation

`Blueprint ID:` `revenue_retention_simulation`
`Category:` `Business`
...
```bash
mn run revenue_retention_simulation
```
...
Run from the catalog with `mn run revenue_retention_simulation` or from this folder with `mn run --folder .`.
