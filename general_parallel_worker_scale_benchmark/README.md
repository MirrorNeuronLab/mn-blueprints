# Parallel Worker Scale Benchmark

`Blueprint ID:` `general_parallel_worker_scale_benchmark`  
...
Stress the runtime with a broad fan-out of deterministic parallel workers.
...
1. Load the graph in `manifest.json` and start `dispatcher` with bundled mock inputs.
2. Route work through the agents described by the manifest, using parallel synthetic workload sweep as the evolving system.
3. Let the `Parallel executor workers plus reducer` observe intermediate state, produce decisions or artifacts, and emit typed messages.
4. Preserve execution metadata, logs, and generated artifacts so users can audit what happened.
5. Return scale summary and worker completion metrics for review, customization, or downstream workflow integration.

## Example scenario

The runtime fans out numeric range checks, collects worker outputs, and summarizes completion across many executor jobs.
...
Change worker count, payload size, task duration, or reducer logic to match your capacity benchmark.
...
Scale and reliability are commercial proof points; this blueprint makes orchestration capacity visible. The product lesson is that this is not just a chatbot around data. It is a repeatable pattern for vertical workflows where simulation, state, and agent decisions create a more defensible user experience than static analytics alone.
