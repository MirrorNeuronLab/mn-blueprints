...
`Blueprint ID:` `general_human_in_the_loop_workflow`  
...
Run a full human-in-the-loop decision workflow with review requests, responses, revisions, and auditable applied actions.
...
1. Load mock or adapter-provided inputs for state-driven human-in-the-loop decision workflow.
...
4. Ask the `Human-in-the-loop workflow coordinator` to observe the current state, compare valid actions, and choose among `request_human_review`, `apply_reviewer_revision`, `hold_policy`.
5. Apply the selected action back into the simulated system so the next observation changes.
6. Write a structured final artifact for `human review workflow memo` with action history, state deltas, ranked options, and next steps.

## Example scenario

Risk exposure rises, the agent recommends mitigation, a review request is emitted, and the human response determines whether the action is applied, revised, or held.
...
| `final_artifact` | The user-facing human review workflow memo with recommended and applied actions. | `recommended_action`, `ranked_options`, `next_steps` |
...
```bash
cd general_human_in_the_loop_workflow
...
- human input request/response
- LLM recommendation
- timeout and revision handling
- audit trail
...
- Add human input request/responses for high-impact actions.
