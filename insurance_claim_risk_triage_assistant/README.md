# Insurance Claim Risk Triage Assistant

`Blueprint ID:` `insurance_claim_risk_triage_assistant`
`Category:` `Finance`

Use this blueprint to prioritize claim reviews by combining fraud signals, queue pressure, and adjuster capacity into an auditable triage recommendation.
...
```bash
mn run insurance_claim_risk_triage_assistant
```
...
Run from the catalog with `mn run insurance_claim_risk_triage_assistant` or from this folder with `mn run --folder .`.
