# Portfolio Crash Stress Simulation

`Blueprint ID:` `portfolio_crash_stress_simulation`
`Category:` `Finance`

Use this blueprint to stress a portfolio against crash, rate, and liquidity shocks and review rebalancing choices before taking action.
...
```bash
mn run portfolio_crash_stress_simulation
```
...
Run from the catalog with `mn run portfolio_crash_stress_simulation` or from this folder with `mn run --folder .`.
