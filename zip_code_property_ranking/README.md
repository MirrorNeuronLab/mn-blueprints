...
Use this blueprint to rank property acquisition opportunities by ZIP-code demand, price, cap-rate, and risk signals before committing diligence time.
...
```bash
mn run zip_code_property_ranking
```
...
Run from the catalog with `mn run zip_code_property_ranking` or from this folder with `mn run --folder .`.
