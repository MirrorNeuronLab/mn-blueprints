# Revenue Retention Planning Assistant

`Blueprint ID:` `revenue_retention_planning_assistant`
`Category:` `Business`

Use this blueprint to compare retention offers for at-risk customers and choose interventions with clear revenue, churn, and customer-experience tradeoffs.
...
```bash
mn run revenue_retention_planning_assistant
```
...
Run from the catalog with `mn run revenue_retention_planning_assistant` or from this folder with `mn run --folder .`.
