...
The blueprint is organized as a MirrorNeuron workflow with stable identity, configurable inputs, structured events, and a final artifact. The main agent role is Python service coordinator. The workflow uses live service loop with repeated inputs and demonstrates Python SDK, service execution, live loop, and runtime events.
...
A representative scenario is: A service periodically collects signals, decides whether more work is needed, and emits runtime events until cancelled.
