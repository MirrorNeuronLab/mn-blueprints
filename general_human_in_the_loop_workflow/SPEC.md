# Human In The Loop Workflow SPEC

## What We Want To Achieve

Build a reviewable security and governance workflow that helps Developers and governance teams designing approval-gated agent workflows move from raw signals to an explainable recommendation. Run a full human-in-the-loop decision workflow with review requests, responses, revisions, and auditable applied actions. The target customer should understand what changed, why the system recommended an action, and what evidence a human should review before acting.

## Customer Problem

Enterprise agents often recommend actions that should not execute automatically; approval state must be part of the runtime loop. In a real customer environment, the pain is not only producing an answer; it is preserving context across changing inputs, exposing tradeoffs, and creating an audit trail that business, technical, or governance stakeholders can trust.

## Design Details

The blueprint is organized as a MirrorNeuron workflow with stable identity, configurable inputs, structured events, and a final artifact. The main agent role is Human-in-the-loop workflow coordinator. The workflow uses state-driven human-in-the-loop decision workflow and demonstrates human input request/response, LLM recommendation, timeout and revision handling, and audit trail.
...
A representative scenario is: Risk exposure rises, the agent recommends mitigation, a review request is emitted, and the human response determines whether the action is applied, revised, or held.
...
Important state inputs include `risk_exposure`, `approval_latency`, `benefit_score`, and `budget_used`. Where the blueprint uses an action loop, the current action space includes `request_human_review`, `apply_reviewer_revision`, and `hold_policy`. For production use, the same contract should be fed by customer system-of-record data, business rules, approval policies, thresholds, and any regulated or safety-critical constraints needed for the operating environment.

## Output: Expected Customer Outcome

The expected customer outcome is human review workflow memo with recommended and applied actions. A useful run should show the starting context, the observations made during the workflow, the action or recommendation rationale, and the final artifact that a domain owner can review.
...
- State trajectory: inspect whether `risk_exposure`, `approval_latency`, `benefit_score`, and `budget_used` move coherently across the workflow rather than appearing as disconnected summaries.
...
Replace the approval policy, add approver roles, connect ticketing systems, and define which actions require human sign-off. Add customer-specific policies, review gates, exception handling, retention rules, and monitoring dashboards. Calibrate the workflow against historical data and expert judgment, then track acceptance rate, correction rate, latency, incident reduction, cost impact, and other outcome metrics that prove whether the workflow is helping.

## Product Narrative

Human-in-the-loop control is essential for enterprise adoption, especially in regulated or high-cost workflows.
