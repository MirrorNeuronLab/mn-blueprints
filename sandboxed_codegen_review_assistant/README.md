# Sandboxed Codegen Review Assistant

`Blueprint ID:` `sandboxed_codegen_review_assistant`
`Category:` `Security`

Use this blueprint to generate, review, and validate code inside a sandboxed LLM loop before trusting the result.
...
```bash
mn run sandboxed_codegen_review_assistant
```
...
Run from the catalog with `mn run sandboxed_codegen_review_assistant` or from this folder with `mn run --folder .`.
