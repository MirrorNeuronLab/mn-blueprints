# Insurance Claim Risk Triage Assistant SPEC

## What We Want To Achieve

Build a reviewable financial decision-support workflow that helps Insurance claims leaders, SIU teams, adjuster operations, and insurtech builders move from raw signals to an explainable recommendation. Prioritize insurance claim reviews as fraud signals, queue pressure, and adjuster capacity change. The target customer should understand what changed, why the system recommended an action, and what evidence a human should review before acting.

## Customer Problem

Claims queues evolve continuously; teams need to balance fast settlement, fraud risk, loss ratio, and adjuster capacity. In a real customer environment, the pain is not only producing an answer; it is preserving context across changing inputs, exposing tradeoffs, and creating an audit trail that business, technical, or governance stakeholders can trust.

## Design Details

The blueprint is organized as a MirrorNeuron workflow with stable identity, configurable inputs, structured events, and a final artifact. The main agent role is Insurance claim risk triage agent. The workflow uses claim queue and fraud-signal simulation and demonstrates claim stream simulation, risk triage, LLM prioritization, and feedback loop.
...
A representative scenario is: Auto claims volume rises, fraud signal spikes, and the agent chooses SIU escalation, fast-track, or evidence requests.
...
Important state inputs include `claim_queue`, `fraud_signal`, `adjuster_capacity`, and `loss_ratio_pct`. Where the blueprint uses an action loop, the current action space includes `escalate_siu`, `fast_track_low_risk`, and `request_evidence`. For production use, the same contract should be fed by customer system-of-record data, business rules, approval policies, thresholds, and any regulated or safety-critical constraints needed for the operating environment.

## Output: Expected Customer Outcome

The expected customer outcome is claim triage report with prioritized segments and review actions. A useful run should show the starting context, the observations made during the workflow, the action or recommendation rationale, and the final artifact that a domain owner can review.
...
- State trajectory: inspect whether `claim_queue`, `fraud_signal`, `adjuster_capacity`, and `loss_ratio_pct` move coherently across the workflow rather than appearing as disconnected summaries.
...
Replace synthetic claims with FNOL, adjuster notes, photos, repair estimates, policy data, and fraud-model outputs. Add customer-specific policies, review gates, exception handling, retention rules, and monitoring dashboards. Calibrate the workflow against historical data and expert judgment, then track acceptance rate, correction rate, latency, incident reduction, cost impact, and other outcome metrics that prove whether the workflow is helping.

## Product Narrative

Claims triage has measurable ROI in leakage, cycle time, and customer satisfaction, making it a strong vertical agent workflow.
