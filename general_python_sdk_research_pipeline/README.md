# Python Sdk Research Pipeline

`Blueprint ID:` `general_python_sdk_research_pipeline`  
...
Define a MirrorNeuron workflow directly in Python and compile it into a runnable bundle.
...
Python developers who want agent workflows without hand-writing manifests.

## Why it matters

Developers need a low-friction path from Python functions to distributed runtime agents while keeping retry, timeout, and graph semantics explicit. A static dashboard can show the current state, and a one-shot LLM prompt can summarize it, but neither tests what happens after a decision is applied. This blueprint makes the feedback loop visible: state changes, the agent observes it, the agent chooses an action, and the system evolves again.
...
2. Route work through the agents described by the manifest, using deterministic staged workflow as the evolving system.
3. Let the `Python-defined research agents` observe intermediate state, produce decisions or artifacts, and emit typed messages.
4. Preserve execution metadata, logs, and generated artifacts so users can audit what happened.
5. Return saved research summary and final workflow result for review, customization, or downstream workflow integration.

## Example scenario

A research topic flows through ingress, retrieval, and review agents before a final summary artifact is saved.
...
| Final artifact | The user-facing saved research summary and final workflow result. | `result.json`, report, alert, or generated artifact |
...
Run the Python workflow directly in a local virtual environment:

```bash
cd general_python_sdk_research_pipeline
python3 -m venv .venv
. .venv/bin/activate
...
Edit `workflow.py`, swap in your own data retrieval function, add more agents, and regenerate the bundle with `mn_blueprint_support.python_workflow_bundle_cli`. For a new Python-defined blueprint, add a `metadata.python_workflow` block to `manifest.json` instead of copying custom generation logic.
...
The SDK path lowers adoption friction: teams can start from normal Python and graduate to runtime-managed agent graphs. The product lesson is that this is not just a chatbot around data. It is a repeatable pattern for vertical workflows where simulation, state, and agent decisions create a more defensible user experience than static analytics alone.
