# Demo: State Audit Workflow

`Blueprint ID:` `demo_state_audit_workflow`
`Category:` `Engineering`

Use this blueprint to record simulation state changes so every agent decision can be inspected after the run.
...
```bash
mn run demo_state_audit_workflow
```
...
Run from the catalog with `mn run demo_state_audit_workflow` or from this folder with `mn run --folder .`.
