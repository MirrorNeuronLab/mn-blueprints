# Claim Risk Triage Copilot

`Blueprint ID:` `finance_claim_risk_triage_copilot`  
...
Prioritize insurance claim reviews as fraud signals, queue pressure, and adjuster capacity change.
...
Insurance claims leaders, SIU teams, adjuster operations, and insurtech builders.

## Why it matters

Claims queues evolve continuously; teams need to balance fast settlement, fraud risk, loss ratio, and adjuster capacity. A static dashboard can show the current state, and a one-shot LLM prompt can summarize it, but neither tests what happens after a decision is applied. This blueprint makes the feedback loop visible: state changes, the agent observes it, the agent chooses an action, and the system evolves again.
...
1. Load mock or adapter-provided inputs for claim queue and fraud-signal simulation.
2. Initialize time-varying state with metrics such as `claim_queue`, `fraud_signal`, `adjuster_capacity`, `loss_ratio_pct`.
3. Advance the environment one step with deterministic seeded drift and volatility.
4. Ask the `Insurance claim risk triage agent` to observe the current state, compare valid actions, and choose among `escalate_siu`, `fast_track_low_risk`, `request_evidence`.
5. Apply the selected action back into the simulated system so the next observation changes.
6. Write a structured final artifact for `claim triage report` with action history, state deltas, ranked options, and next steps.

## Example scenario

Auto claims volume rises, fraud signal spikes, and the agent chooses SIU escalation, fast-track, or evidence requests.
...
| `line` | Scenario control, seed, or domain input. | "personal auto" | Yes |
| `initial_claim_queue` | Override the starting value for open claim queue. | 95 | Yes |
| `initial_fraud_signal` | Override the starting value for fraud signal. | 42 | Yes |
| `initial_adjuster_capacity` | Override the starting value for adjuster capacity. | 70 | Yes |
| `initial_loss_ratio_pct` | Override the starting value for loss ratio percent. | 64 | Yes |
...
| `final_artifact` | The user-facing claim triage report with prioritized segments and review actions. | `recommended_action`, `ranked_options`, `next_steps` |
...
```bash
cd finance_claim_risk_triage_copilot
...
Replace synthetic claims with FNOL, adjuster notes, photos, repair estimates, policy data, and fraud-model outputs.
...
Inspect `timeline` to see each observation and decision, `state_changes` to verify the simulated `claim_queue` and `fraud_signal` moved over time, and `final_artifact` to review the recommended action and ranked claim segment options.
...
Claims triage has measurable ROI in leakage, cycle time, and customer satisfaction, making it a strong vertical agent workflow. The product lesson is that this is not just a chatbot around data. It is a repeatable pattern for vertical workflows where simulation, state, and agent decisions create a more defensible user experience than static analytics alone.
