# Liquidity Risk Monitoring Assistant

`Blueprint ID:` `liquidity_risk_monitoring_assistant`
`Category:` `Finance`

Use this blueprint to monitor market microstructure signals and understand emerging liquidity risk before trades become expensive or fragile.
...
```bash
mn run liquidity_risk_monitoring_assistant
```
...
Run from the catalog with `mn run liquidity_risk_monitoring_assistant` or from this folder with `mn run --folder .`.
