# Demo: Context Memory Audit

`Blueprint ID:` `demo_context_memory_audit`
`Category:` `Engineering`

Use this blueprint to audit how role-specific context packets move through a multi-agent decision process and affect the final recommendation.
...
```bash
mn run demo_context_memory_audit
```
...
Run from the catalog with `mn run demo_context_memory_audit` or from this folder with `mn run --folder .`.
