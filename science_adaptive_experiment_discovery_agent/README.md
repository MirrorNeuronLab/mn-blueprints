# Adaptive Experiment Discovery Agent

`Blueprint ID:` `science_adaptive_experiment_discovery_agent`  
...
Select the next lab experiment from prior simulated yield, toxicity, cost, and evidence confidence.
...
Lab researchers, R&D teams, computational scientists, and scientific AI builders.

## Why it matters

Experiment planning is sequential; each result changes the next best experiment and the cost-risk tradeoff. A static dashboard can show the current state, and a one-shot LLM prompt can summarize it, but neither tests what happens after a decision is applied. This blueprint makes the feedback loop visible: state changes, the agent observes it, the agent chooses an action, and the system evolves again.
...
1. Load mock or adapter-provided inputs for iterative experiment outcome simulation.
2. Initialize time-varying state with metrics such as `yield_pct`, `toxicity_score`, `experiment_cost`, `evidence_confidence`.
3. Advance the environment one step with deterministic seeded drift and volatility.
4. Ask the `Experimental design scientist` to observe the current state, compare valid actions, and choose among `explore_high_temp`, `refine_ph`, `switch_catalyst`.
5. Apply the selected action back into the simulated system so the next observation changes.
6. Write a structured final artifact for `experiment optimization plan` with action history, state deltas, ranked options, and next steps.

## Example scenario

A synthetic enzyme candidate shows mixed yield and toxicity, and the scientist agent chooses pH refinement, temperature sweep, or catalyst switch.
...
| `compound_family` | Scenario control, seed, or domain input. | "synthetic enzyme candidate" | Yes |
| `initial_yield_pct` | Override the starting value for experimental yield percent. | 37 | Yes |
| `initial_toxicity_score` | Override the starting value for toxicity score. | 31 | Yes |
| `initial_experiment_cost` | Override the starting value for experiment cost index. | 12 | Yes |
| `initial_evidence_confidence` | Override the starting value for evidence confidence. | 52 | Yes |
...
| `final_artifact` | The user-facing experiment optimization plan with ranked experiment options. | `recommended_action`, `ranked_options`, `next_steps` |
...
```bash
cd science_adaptive_experiment_discovery_agent
...
Replace synthetic outcomes with LIMS/ELN data, assay results, cost models, safety constraints, and Bayesian or active-learning tools.
...
Inspect `timeline` to see each observation and decision, `state_changes` to verify the simulated `yield_pct` and `toxicity_score` moved over time, and `final_artifact` to review the recommended action and ranked experiment options.
