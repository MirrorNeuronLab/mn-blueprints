...
`Blueprint ID:` `drug_discovery_simulation`
`Category:` `Science`

Use this blueprint to run a multi-stage drug discovery loop that generates, filters, and evaluates candidates with reviewable evidence.
...
```bash
mn run drug_discovery_simulation
```
...
Run from the catalog with `mn run drug_discovery_simulation` or from this folder with `mn run --folder .`.
