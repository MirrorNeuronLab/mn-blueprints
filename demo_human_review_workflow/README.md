# Demo: Human Review Workflow

`Blueprint ID:` `demo_human_review_workflow`
`Category:` `Security`

Use this blueprint to run a complete human review loop where requests, decisions, revisions, and applied actions are all captured.
...
```bash
mn run demo_human_review_workflow
```
...
Run from the catalog with `mn run demo_human_review_workflow` or from this folder with `mn run --folder .`.
