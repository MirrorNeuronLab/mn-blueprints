# Traffic Signal Control Assistant SPEC

## What We Want To Achieve

Build a reviewable scientific planning workflow that helps City traffic teams, transportation researchers, smart-city operators, and mobility startups move from raw signals to an explainable recommendation. Test signal and rerouting controls as traffic speed, incidents, volume, and emissions evolve. The target customer should understand what changed, why the system recommended an action, and what evidence a human should review before acting.

## Customer Problem

Traffic control decisions are coupled and time-sensitive; dashboards show congestion but do not reason over next-state control options. In a real customer environment, the pain is not only producing an answer; it is preserving context across changing inputs, exposing tradeoffs, and creating an audit trail that business, technical, or governance stakeholders can trust.

## Design Details

The blueprint is organized as a MirrorNeuron workflow with stable identity, configurable inputs, structured events, and a final artifact. The main agent role is Traffic systems control advisor. The workflow uses traffic network and incident-control simulation and demonstrates traffic simulation, control strategy, LLM advisor, and time-varying state.
...
A representative scenario is: Vehicle count rises and an incident worsens, so the agent selects green corridors, rerouting, or transit priority.
...
Important state inputs include `vehicle_count`, `avg_speed_mph`, `incident_severity`, and `emissions_index`. Where the blueprint uses an action loop, the current action space includes `extend_green_corridor`, `reroute_incident`, and `transit_priority`. For production use, the same contract should be fed by customer system-of-record data, business rules, approval policies, thresholds, and any regulated or safety-critical constraints needed for the operating environment.

## Output: Expected Customer Outcome

The expected customer outcome is traffic control plan with corridor ranking and action history. A useful run should show the starting context, the observations made during the workflow, the action or recommendation rationale, and the final artifact that a domain owner can review.
...
- State trajectory: inspect whether `vehicle_count`, `avg_speed_mph`, `incident_severity`, and `emissions_index` move coherently across the workflow rather than appearing as disconnected summaries.
...
Connect loop detectors, cameras, signal controllers, incident feeds, GTFS transit data, and emissions models. Add customer-specific policies, review gates, exception handling, retention rules, and monitoring dashboards. Calibrate the workflow against historical data and expert judgment, then track acceptance rate, correction rate, latency, incident reduction, cost impact, and other outcome metrics that prove whether the workflow is helping.

## Product Narrative

Urban mobility is a visible vertical where simulation plus AI control recommendations can become a city operations product.
