# Sandboxed Llm Codegen Review Loop

`Blueprint ID:` `general_sandboxed_llm_codegen_review_loop`  
...
Generate, review, and validate code through a sandboxed multi-step LLM loop.
...
1. Load the graph in `manifest.json` and start `codegen_round_1` with bundled mock inputs.
2. Route work through the agents described by the manifest, using iterative code-review decision loop as the evolving system.
3. Let the `Codegen agent, review agent, validator` observe intermediate state, produce decisions or artifacts, and emit typed messages.
4. Preserve execution metadata, logs, and generated artifacts so users can audit what happened.
5. Return generated code, review history, and validation result for review, customization, or downstream workflow integration.

## Example scenario

The LLM writes a Python tool, another LLM review step critiques it, and a validator executes tests inside an OpenShell sandbox.

## Inputs

| Input | What it controls | Example | Can customize? |
|---|---|---|---|
| `manifest.json` initial inputs | Sample payloads routed into codegen_round_1. | `initial_inputs` | Yes |
...
| Final artifact | The user-facing generated code, review history, and validation result. | `result.json`, report, alert, or generated artifact |
...
```bash
cd general_sandboxed_llm_codegen_review_loop
mn blueprint run .
```
...
Change the coding task, validator tests, review rubric, sandbox policy, model, or allowed toolchain.
...
This is a credible path to enterprise coding agents because it combines reasoning, iteration, and sandboxed execution controls. The product lesson is that this is not just a chatbot around data. It is a repeatable pattern for vertical workflows where simulation, state, and agent decisions create a more defensible user experience than static analytics alone.
