# Event Stream Triage Assistant

`Blueprint ID:` `event_stream_triage_assistant`
`Category:` `Engineering`

Use this blueprint to triage noisy event streams and see how anomaly pressure, queue depth, and false positives change decisions over time.
...
```bash
mn run event_stream_triage_assistant
```
...
Run from the catalog with `mn run event_stream_triage_assistant` or from this folder with `mn run --folder .`.
