# Demo: Message Routing Trace

`Blueprint ID:` `demo_message_routing_trace`
`Category:` `Engineering`

Use this blueprint to trace how messages move through router and aggregator agents so workflow wiring is easier to debug and explain.
...
```bash
mn run demo_message_routing_trace
```
...
Run from the catalog with `mn run demo_message_routing_trace` or from this folder with `mn run --folder .`.
