# Property Deal Memory Research Assistant

`Blueprint ID:` `property_deal_memory_research_assistant`
`Category:` `Finance`

Use this blueprint to rank property opportunities while preserving useful deal memory across noisy ZIP-code history, broker flow, financing constraints, and past outcomes.
...
```bash
mn run property_deal_memory_research_assistant
```
...
Run from the catalog with `mn run property_deal_memory_research_assistant` or from this folder with `mn run --folder .`.
