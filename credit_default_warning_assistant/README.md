# Credit Default Warning Assistant

`Blueprint ID:` `credit_default_warning_assistant`
`Category:` `Finance`

Use this blueprint to spot borrower default pressure early and compare policy responses before credit risk turns into losses.
...
```bash
mn run credit_default_warning_assistant
```
...
Run from the catalog with `mn run credit_default_warning_assistant` or from this folder with `mn run --folder .`.
