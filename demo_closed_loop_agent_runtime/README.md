# Demo: Closed Loop Agent Runtime

`Blueprint ID:` `demo_closed_loop_agent_runtime`
`Category:` `Engineering`

Use this blueprint to understand how a closed-loop agent observes changing work, decides on each tick, and leaves an inspectable action trail.
...
```bash
mn run demo_closed_loop_agent_runtime
```
...
Run from the catalog with `mn run demo_closed_loop_agent_runtime` or from this folder with `mn run --folder .`.
