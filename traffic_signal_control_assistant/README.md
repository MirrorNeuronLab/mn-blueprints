# Traffic Signal Control Assistant

`Blueprint ID:` `traffic_signal_control_assistant`
`Category:` `Science`

Use this blueprint to test traffic signal and rerouting controls against speed, incident, volume, and emissions tradeoffs.
...
```bash
mn run traffic_signal_control_assistant
```
...
Run from the catalog with `mn run traffic_signal_control_assistant` or from this folder with `mn run --folder .`.
