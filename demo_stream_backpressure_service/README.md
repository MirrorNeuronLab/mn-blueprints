# Demo: Stream Backpressure Service

`Blueprint ID:` `demo_stream_backpressure_service`
`Category:` `Engineering`

Use this blueprint to observe bounded queues, slow workers, and retry-later behavior before building live stream workflows.
...
```bash
mn run demo_stream_backpressure_service
```
...
Run from the catalog with `mn run demo_stream_backpressure_service` or from this folder with `mn run --folder .`.
