...
`Blueprint ID:` `contract_negotiation_simulation`
...
```bash
mn run contract_negotiation_simulation
```
...
Run from the catalog with `mn run contract_negotiation_simulation` or from this folder with `mn run --folder .`.
