# Demo: LLM Tool Orchestration

`Blueprint ID:` `demo_llm_tool_orchestration`
`Category:` `Engineering`

Use this blueprint to see an LLM agent call a forecast tool, weigh the result, and choose a resource action you can inspect.
...
```bash
mn run demo_llm_tool_orchestration
```
...
Run from the catalog with `mn run demo_llm_tool_orchestration` or from this folder with `mn run --folder .`.
