# Zip Code Property Memory Ranking

`Blueprint ID:` `zip_code_property_memory_ranking`
`Category:` `Finance`
...
```bash
mn run zip_code_property_memory_ranking
```
...
Run from the catalog with `mn run zip_code_property_memory_ranking` or from this folder with `mn run --folder .`.
