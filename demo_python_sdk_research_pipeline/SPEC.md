# Demo: Python SDK Research Pipeline SPEC

## What We Want To Achieve

Build a reviewable runtime evaluation workflow that helps Python developers who want agent workflows without hand-writing manifests move from raw signals to an explainable recommendation. Define a MirrorNeuron workflow directly in Python and compile it into a runnable bundle. The target customer should understand what changed, why the system recommended an action, and what evidence a human should review before acting.

## Customer Problem

Developers need a low-friction path from Python functions to distributed runtime agents while keeping retry, timeout, and graph semantics explicit. In a real customer environment, the pain is not only producing an answer; it is preserving context across changing inputs, exposing tradeoffs, and creating an audit trail that business, technical, or governance stakeholders can trust.

## Design Details

The blueprint is organized as a MirrorNeuron workflow with stable identity, configurable inputs, structured events, and a final artifact. The main agent role is Python-defined research agents. The workflow uses deterministic staged workflow and demonstrates Python SDK, map/reduce agents, compiled bundle, and retry policies.
...
A representative scenario is: A research topic flows through ingress, retrieval, and review agents before a final summary artifact is saved.
...
The expected customer outcome is saved research summary and final workflow result. A useful run should show the starting context, the observations made during the workflow, the action or recommendation rationale, and the final artifact that a domain owner can review.
...
Edit `workflow.py`, swap in your own data retrieval function, add more agents, and regenerate the bundle with `python -m mn_blueprint_support.python_workflow_bundle_cli`. Add customer-specific policies, review gates, exception handling, retention rules, and monitoring dashboards. Calibrate the workflow against historical data and expert judgment, then track acceptance rate, correction rate, latency, incident reduction, cost impact, and other outcome metrics that prove whether the workflow is helping.

## Product Narrative

The SDK path lowers adoption friction: teams can start from normal Python and graduate to runtime-managed agent graphs.
