# Lab Experiment Planning Assistant

`Blueprint ID:` `lab_experiment_planning_assistant`
`Category:` `Science`

Use this blueprint to choose the next experiment from prior yield, toxicity, cost, and confidence signals before spending lab capacity.
...
```bash
mn run lab_experiment_planning_assistant
```
...
Run from the catalog with `mn run lab_experiment_planning_assistant` or from this folder with `mn run --folder .`.
