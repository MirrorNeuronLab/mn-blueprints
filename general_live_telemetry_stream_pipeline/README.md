# Live Telemetry Stream Pipeline

`Blueprint ID:` `general_live_telemetry_stream_pipeline`  
...
Move live telemetry chunks through stream workers and summarize the evolving signal.
...
Developers building event-stream workflows and runtime observability paths.

## Why it matters

Streaming systems require backpressure-aware routing, stateful workers, and outputs that remain interpretable as events arrive. A static dashboard can show the current state, and a one-shot LLM prompt can summarize it, but neither tests what happens after a decision is applied. This blueprint makes the feedback loop visible: state changes, the agent observes it, the agent chooses an action, and the system evolves again.
...
2. Route work through the agents described by the manifest, using live telemetry stream as the evolving system.
3. Let the `Stream processor and summarizer` observe intermediate state, produce decisions or artifacts, and emit typed messages.
4. Preserve execution metadata, logs, and generated artifacts so users can audit what happened.
5. Return signal summary and stream result artifacts for review, customization, or downstream workflow integration.

## Example scenario

Synthetic telemetry events are generated, processed by a stream worker, and summarized into signal peaks and state changes.
...
| Final artifact | The user-facing signal summary and stream result artifacts. | `result.json`, report, alert, or generated artifact |
...
```bash
cd general_live_telemetry_stream_pipeline
mn blueprint run .
```
...
Replace the telemetry generator, add stream partitions, tune event cadence, and extend the summarizer for your metrics.
...
Stream processing is where static prompts fail; this blueprint shows agents embedded in live data movement. The product lesson is that this is not just a chatbot around data. It is a repeatable pattern for vertical workflows where simulation, state, and agent decisions create a more defensible user experience than static analytics alone.
