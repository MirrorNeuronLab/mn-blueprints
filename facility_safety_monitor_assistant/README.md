# Facility Safety Monitor Assistant

`Blueprint ID:` `facility_safety_monitor_assistant`
`Category:` `Security`

Use this blueprint to watch an approved camera stream, detect visible people, summarize what is observable, and raise safety alerts with a reviewable event trail.
...
```bash
mn run facility_safety_monitor_assistant
```
...
Run from the catalog with `mn run facility_safety_monitor_assistant` or from this folder with `mn run --folder .`.
