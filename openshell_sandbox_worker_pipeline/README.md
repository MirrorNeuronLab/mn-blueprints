...
Use this blueprint to run shell and Python workers inside isolated execution boundaries while keeping each step traceable.
...
```bash
mn run openshell_sandbox_worker_pipeline
```
...
Run from the catalog with `mn run openshell_sandbox_worker_pipeline` or from this folder with `mn run --folder .`.
