# Multi Agent Motion Planning Lab

`Blueprint ID:` `science_multi_agent_motion_planning_lab`  
...
Visualize shared-world multi-agent motion planning and coordination behavior.
...
Robotics researchers, simulation developers, and multi-agent systems educators.

## Why it matters

Multi-agent coordination depends on changing positions, goals, and collisions; users need visual traces, not only scalar metrics. A static dashboard can show the current state, and a one-shot LLM prompt can summarize it, but neither tests what happens after a decision is applied. This blueprint makes the feedback loop visible: state changes, the agent observes it, the agent chooses an action, and the system evolves again.
...
2. Route work through the agents described by the manifest, using multi-agent particle environment as the evolving system.
3. Let the `World simulator and visualization summarizer` observe intermediate state, produce decisions or artifacts, and emit typed messages.
4. Preserve execution metadata, logs, and generated artifacts in the shared `~/.mn/runs/<run_id>/` contract so users can audit what happened.
5. Return simulation summary and visual artifacts for review, customization, or downstream workflow integration.

## Example scenario

A shared-world particle environment is generated and summarized to show how agents coordinate around a target task.
...
```bash
cd science_multi_agent_motion_planning_lab
MN_INPUT_FILE=/path/to/input.json \
MN_RUN_ID=motion-debug-001 \
MN_RUNS_ROOT=/tmp/mirror-neuron-runs \
python3 payloads/world_worker/scripts/run_shared_world.py
```

The worker still prints the structured JSON result to stdout for the graph runtime, and also writes `run.json`, `inputs.json`, `events.jsonl`, `result.json`, and `final_artifact.json` when the shared support skill is available.
...
Change environment geometry, agent policies, reward structure, visualization output, and planning constraints.
...
Multi-agent physical simulation is a foundation for robotics and autonomy workflows where stateful agents must coordinate over time. The product lesson is that this is not just a chatbot around data. It is a repeatable pattern for vertical workflows where simulation, state, and agent decisions create a more defensible user experience than static analytics alone.
...
- shared-world state
- summary generation
- shared run-store contract for specialized workers
- event-stream observability over simulation cycles
