...
`Blueprint ID:` `traffic_control_simulation`
`Category:` `Science`
...
```bash
mn run traffic_control_simulation
```
...
Run from the catalog with `mn run traffic_control_simulation` or from this folder with `mn run --folder .`.
