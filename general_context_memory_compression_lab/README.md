# Context Memory Compression Lab

`Blueprint ID:` `general_context_memory_compression_lab`  
...
Stress-test compressed working memory across a growing multi-agent LLM workflow.
...
Developers building long-context agent systems with bounded prompt budgets.

## Why it matters

Long-running agents accumulate more context than a model can safely consume; compression must preserve pinned facts, constraints, and decision traces. A static dashboard can show the current state, and a one-shot LLM prompt can summarize it, but neither tests what happens after a decision is applied. This blueprint makes the feedback loop visible: state changes, the agent observes it, the agent chooses an action, and the system evolves again.
...
2. Route work through the agents described by the manifest, using growing memory graph with compression pressure as the evolving system.
3. Let the `Context researcher, expander, challenger, compressor, briefing author` observe intermediate state, produce decisions or artifacts, and emit typed messages.
4. Preserve execution metadata, logs, and generated artifacts so users can audit what happened.
5. Return compressed context handoff and final briefing for review, customization, or downstream workflow integration.

## Example scenario

Agents create larger artifacts over several turns while CompileContext keeps each packet bounded and traceable.
...
| Final artifact | The user-facing compressed context handoff and final briefing. | `result.json`, report, alert, or generated artifact |
...
```bash
mn blueprint run general_context_memory_compression_lab
```
...
Change token budgets, pinned terms, memory item types, compression policy, and model-compression settings.
...
State management and compression are differentiators for agent platforms moving beyond one-shot prompts. The product lesson is that this is not just a chatbot around data. It is a repeatable pattern for vertical workflows where simulation, state, and agent decisions create a more defensible user experience than static analytics alone.
