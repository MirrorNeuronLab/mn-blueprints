# Ecosystem Intervention Planning Assistant SPEC

## What We Want To Achieve

Build a reviewable scientific planning workflow that helps Ecologists, conservation analysts, simulation researchers, and education teams move from raw signals to an explainable recommendation. Explore ecosystem interventions across multiple regions with evolving population dynamics. The target customer should understand what changed, why the system recommended an action, and what evidence a human should review before acting.

## Customer Problem

Ecological systems change through local interactions, migration, resources, and mutation; interventions need scenario testing before field action. In a real customer environment, the pain is not only producing an answer; it is preserving context across changing inputs, exposing tradeoffs, and creating an audit trail that business, technical, or governance stakeholders can trust.

## Design Details

The blueprint is organized as a MirrorNeuron workflow with stable identity, configurable inputs, structured events, resource/log artifacts, a shared Gradio dashboard, and a final artifact. The dashboard is declared as Grafana-style JSON and rendered by the shared blueprint support skill, so this non-video workflow uses the same web UI contract as stream-oriented blueprints. The main agent role is World coordinator, regional agents, leaderboard summarizer. The workflow uses multi-region population dynamics and demonstrates native agents, multi-region simulation, aggregation, leaderboard reporting, and service-registered web UI publication.
...
A representative scenario is: A world agent distributes animals across regions, region agents evolve local populations, and a leaderboard summarizes outcomes.

## Input

The prototype accepts configuration for scenario identity, run controls, and domain inputs. Current adapters include `mock`, `json`, `file`, and `env_json`, so evaluators can start locally and later replace sample data with production data while preserving the same blueprint identity and output shape. The default mock payload lives under `inputs.payload` and mirrors the `ecosystem` values bound into the manifest.
...
The expected customer outcome is regional outcomes, population rankings, and ecosystem summary. A useful run should show the starting context, the observations made during the workflow, the action or recommendation rationale, and the final artifact that a domain owner can review.
...
When using the local run store, inspect `run.json`, `config.json`, `inputs.json`, `events.jsonl`, `result.json`, and `final_artifact.json`. When the shared Gradio dashboard is active, inspect `web_ui.json` and `ui.json`; for operations, also inspect `logs.jsonl`, `logs.index.json`, `resources.jsonl`, and `human.jsonl` when present. These artifacts are the review surface for debugging the workflow, comparing scenarios, and deciding whether the blueprint is ready for a real adapter.
...
Change species traits, resource regeneration, migration rate, region topology, intervention policies, or final scoring. Add customer-specific policies, review gates, exception handling, retention rules, and monitoring dashboard copy while keeping the shared `blueprint-web-ui` service contract. Calibrate the workflow against historical data and expert judgment, then track acceptance rate, correction rate, latency, incident reduction, cost impact, and other outcome metrics that prove whether the workflow is helping.

## Product Narrative

This demonstrates the runtime's ability to coordinate many local simulators into a global decision view, a pattern relevant beyond ecology.
