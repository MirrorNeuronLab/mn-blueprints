# Climate Resilience Planning Simulation

`Blueprint ID:` `climate_resilience_planning_simulation`
`Category:` `Science`

Use this blueprint to compare local flood mitigation plans as rainfall, water levels, pump capacity, and vulnerable assets change.
...
```bash
mn run climate_resilience_planning_simulation
```
...
Run from the catalog with `mn run climate_resilience_planning_simulation` or from this folder with `mn run --folder .`.
